    ...
    <>
      ...
      <ToastContainer
        ...
        icon={({ type }) => {
          ...
          switch (type) {
            case 'success': {
              return <div className="i-ph:check-bold text-cosmiq-elements-icon-success text-2xl" />;
            }
            case 'error': {
              return <div className="i-ph:warning-circle-bold text-cosmiq-elements-icon-error text-2xl" />;
            }
          }
  ...
  ({ description, initialMessages, storeMessageHistory, importChat, exportChat }: ChatProps) => {
    ...
    const sendMessage = async (_event: React.UIEvent, messageInput?: string) => {
      ...
      if (selectedElement) {
        ...
        const elementInfo = `<div class=\"__cosmiqSelectedElement__\" data-element='${JSON.stringify(selectedElement)}'>${JSON.stringify(`${selectedElement.displayText}`)}</div>`;
