  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__cosmiqThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__cosmiqArtifact__', '__cosmiqThought__', '__cosmiqQuickAction', '__cosmiqSelectedElement__'],

      // ['className', '__cosmiqThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__cosmiqArtifact__', '__cosmiqThought__', '__cosmiqQuickAction'],
    ],
  },
  strip: [],
};
